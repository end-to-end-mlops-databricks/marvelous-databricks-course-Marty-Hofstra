...
def test_split_data(mock_read_UC_spark, mock_dataframe, spark_session: SparkSession):
    ...
    assert type(train).__name__ == "DataFrame" # Workaround since isInstance(train, DataFrame) didn't work with databricks-connect
    assert type(test).__name__ == "DataFrame" # Workaround since isInstance(test, DataFrame) didn't work with databricks-connect
